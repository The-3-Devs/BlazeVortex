...
const handleAdminCommands = async (message: Message) => {
    ...
    if (!config.admins.includes(message.author.id)) return message.author.send("you no admin, bitch fuck you, ligma balls");   // not an admin
...
client.on("messageCreate", async (message: Message) => {
    ...
    if (message.content.startsWith("!ignore") || message.content.startsWith("!i")) return;
...
Request: ${content}`;
    ...
    const memoryPath2 = path.join(__dirname, "memory", `${message.author.id}.txt`);
